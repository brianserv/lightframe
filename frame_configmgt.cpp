...
int32_t CFrameConfigMgt::Initialize()
{
	...
	nRet = m_stFrameMessageConfig.GetConfigObject().Initialize();
	if(nRet < 0)
	{
		g_FrameLogEngine.WriteBaseLog(enmLogLevel_Error, "Error When Initialize FrameMessageConfig {message.xml}{ret=0x%x}\n", nRet);
		return nRet;
	}
...
int32_t CFrameConfigMgt::Resume()
{
	...
	nRet = m_stFrameMessageConfig.GetConfigObject().Reload();
	if(nRet < 0)
	{
		g_FrameLogEngine.WriteBaseLog(enmLogLevel_Error, "Error When Initialize FrameMessageConfig {message.xml}{ret=0x%x}\n", nRet);
		return nRet;
